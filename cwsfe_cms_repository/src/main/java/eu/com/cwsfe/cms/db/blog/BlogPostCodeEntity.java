...
@NamedQuery(name = BlogPostCodeEntity.TOTAL_NUMBER_NOT_DELETED_QUERY, query = "SELECT count(bpc) FROM BlogPostCodeEntity bpc WHERE status <> 'DELETED'")
@NamedQuery(name = BlogPostCodeEntity.SEARCH_BY_AJAX_QUERY, query = "SELECT bpc FROM BlogPostCodeEntity bpc where status <> 'DELETED' and blogPostId = :blogPostId order by codeId desc")
@NamedQuery(name = BlogPostCodeEntity.SEARCH_BY_AJAX_COUNT_QUERY, query = "SELECT count(bpc) FROM BlogPostCodeEntity bpc where status <> 'DELETED' and blogPostId = :blogPostId order by codeId desc")
